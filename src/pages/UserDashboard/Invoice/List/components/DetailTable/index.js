...
import { apiInvoices } from 'services/apis/invoices';
import { apiUserInvoicesCSV } from 'services/apis/user_invoices';
...
function DetailTable(props) {
  ...
  const [keywords, setKeywords] = useState({ member_id: localStorage.getItem("memberId") });
  ...
  const handleStart = date => {
    setStart(date);
    setKeywords({ member_id: localStorage.getItem("memberId"), update_date__from: Moment(date).format('YYYY-MM-DD'), update_date__to: Moment(end).format('YYYY-MM-DD') });
  };

  const handleEnd = date => {
    setEnd(date);
    setKeywords({ member_id: localStorage.getItem("memberId"), update_date__from: Moment(start).format('YYYY-MM-DD'), update_date__to: Moment(date).format('YYYY-MM-DD') });
  };
          ...
          {(invoices.pagination && (invoices.pagination.total_count > 0)) && invoices.dataset.map(invoice => (
            <TableRow key={invoice.id}>
              <TableCell component="th" scope="row">
                <Link to={{ pathname: "/UserInvoiceModify", state: { id: invoice.id } }} style={{ textDecoration: 'underline' }}>
                  {invoice.farm_id}
                </Link>
              </TableCell>
