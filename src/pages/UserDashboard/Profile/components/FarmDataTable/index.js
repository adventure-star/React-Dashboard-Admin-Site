...
import { apiMemberFarmDataById } from 'services/apis/users';
...
function FarmDataTable(props) {
  ...
  const getMembers = () => {
    ...
    apiMemberFarmDataById(props.id)
      .then(res => {
        console.log('===== res: ', res);
        if (res) {
          setMembers(res);
      ...
      <Table className={classes.table} aria-label="Members">
        <TableHead>
          <TableRow>
            ...
            <TableCell>購入ポイント</TableCell>
            <TableCell>使用ポイント</TableCell>
        ...
        <TableBody>
          {(members.pagination && (members.pagination.total_count > 0)) && members.dataset.map(member => (
            <TableRow key={member.id}>
              ...
              <TableCell>{member.crop_id}</TableCell>
              <TableCell>{member.level}</TableCell>
              <TableCell>{member.status}</TableCell>
              <TableCell>{member.item_point}</TableCell>
              <TableCell>{member.work_point}</TableCell>
              <TableCell>{member.work_point}</TableCell>
...
export default connect(mapStateToProps, mapDispatchToProps)(FarmDataTable);
