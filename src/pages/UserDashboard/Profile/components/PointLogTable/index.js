...
import { apiMemberPointLogsById } from 'services/apis/users';
...
function PointLogTable(props) {
  ...
  const getMembers = () => {
    ...
    apiMemberPointLogsById(Number(props.id))
      .then(res => {
        console.log('===== res: ', res);
        if (res) {
          setMembers(res);
  ...
  return (
    <TableContainer component={Paper} className={classes.tableContainer}>
      <Table className={classes.table} aria-label="Members">
        <TableHead>
          <TableRow>
            <TableCell>ログ日時</TableCell>
            <TableCell>種別</TableCell>
            <TableCell>ポイント数</TableCell>
            <TableCell>購入額</TableCell>
          </TableRow>
        </TableHead>
        <TableBody>
          {(members.pagination && (members.pagination.total_count > 0)) && members.dataset.map(member => (
            <TableRow key={member.id}>
              ...
              <TableCell>{member.purchase_amount}</TableCell>
            </TableRow>
          ))}
        </TableBody>
        <TableFooter>
...
export default connect(mapStateToProps, mapDispatchToProps)(PointLogTable);
