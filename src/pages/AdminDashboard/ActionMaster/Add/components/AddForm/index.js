...
function AddForm(props) {
  ...
  const [cropidstate, setCropIdState] = useState(1);
  const handleCropIdState = event => {
    setCropIdState(event.target.value);
  ...
  const onCreateActionMaster = () => {
    ...
    apiCreateActionMaster(data)
      .then(res => {
        if (res) {
          history.push('/AdminActionMaster')
        }
      })
    ...
    <Card className="p-4 mb-4">
      ...
      <div>
        <div style={{ display: "flex", alignItems: "center" }}>
          <OneInputTextSmall label="作物ID" value={cropid} />
          <ActionDropDown id="type" label="" param="/crop_master/all/" value={cropidstate} changeAction={handleCropIdState} />
        </div>
