...
import { apiCropMasters, apiCropMasterDuplicate, apiCropMastersCSV, apiCropMastersDataReflection } from 'services/apis/cropmasters';
    ...
    <TableContainer component={Paper} className={classes.tableContainer}>
      <Toolbar className={classes.buttonsOuterWrapper}>
        <Box className={classes.buttonsInnerWrapper}>
          ...
          <NormalButton label="データ反映" onClick={dataReflection} />
          <Tooltip arrow title="新規">
            <Link to="/AdminCropMasterAdd">
          ...
          {(cropmasters.pagination && (cropmasters.pagination.total_count > 0)) && cropmasters.dataset.map(cropmaster => (
            <TableRow key={cropmaster.id}>
              <TableCell component="th" scope="row">
                <Link to={{ pathname: "/AdminCropMasterModify", state: { id: cropmaster.id } }} style={{ textDecoration: 'underline' }}>
                  {cropmaster.id}
                </Link>
              </TableCell>
