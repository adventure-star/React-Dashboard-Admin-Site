...
function AddForm() {
  ...
  const onCropMasterCreate = () => {
    ...
    apiCropMasterCreate(data)
      .then(res => {
        if (res) {
          history.push('/AdminCropMasterModify', {id: res.id})
        }
      })
  ...
  return (
    <Card className="p-4 mb-4">
      ...
      <div>
        <OneInputText label="作物ID" value="" disabled/>
        <DropDown id="region" label="地域" param="/enums/list_by_enum_id/?enum_id=regions" value={area} changeAction={handleArea} />
        <OneInputText label="作物名" value={cropname} changeAction={handleCropName} />
        <DropDown id="level" label="レベル" param="/enums/list_by_enum_id/?enum_id=crop.level" value={level} changeAction={handleLevel} type="number" />

        <div className="m-2">
          <span className="pr-4">種別</span>
          <DropDown id="billing" label="課金" param="/enums/list_by_enum_id/?enum_id=crop.chargeType" value={billing} changeAction={handleBilling} />
          <DropDown id="farm" label="農場" param="/enums/list_by_enum_id/?enum_id=farm.type" value={farm} changeAction={handleFarm} />
          <DropDown id="item" label="アイテム" param="/enums/list_by_enum_id/?enum_id=crop.itemType" value={item} changeAction={handleItem} />
        </div>
        ...
        <DropDown id="area" label="ステータス" param="/enums/list_by_enum_id/?enum_id=crop.status" value={status} changeAction={handleStatus} />
      ...
      <div>
        ...
        <SaveModal state={modal} onNo={toggle} onYes={onCropMasterCreate}/>
      </div>
    </Card>
  );
}
