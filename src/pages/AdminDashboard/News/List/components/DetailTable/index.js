...
import { apiNews } from 'services/apis/news';
...
const headCells = [
  { id: 'object_id', numeric: false, disablePadding: false, label: 'タイトル' },
...
function DetailTable() {
  ...
  const getNews = () => {
    ...
    apiNews({ per_page, page_no, order_by, order_type, ...keywords })
      .then(res => {
        console.log('===== res: ', res);
        if (res) {
          setNews(res);
  ...
  return (
    <TableContainer component={Paper} className={classes.tableContainer}>
      ...
      <Table stickyHeader className={classes.table} aria-label="Configs">
        ...
        />
        <TableBody>
          {(news.pagination && (news.pagination.total_count > 0)) && news.dataset.map(newsitem => (
            <TableRow key={newsitem.id}>
              <TableCell component="th" scope="row">
                <Link to={{ pathname: "/AdminNewsModify", state: { id: newsitem.id } }} style={{ textDecoration: 'underline' }}>
                  {newsitem.object_id}:{ReactHtmlParser(newsitem.title)}
                </Link>
              </TableCell>
              <TableCell>{newsitem.start_date}</TableCell>
              <TableCell>{getStatus(newsitem.delete_flag, newsitem.start_date, newsitem.expire_date)}</TableCell>
            </TableRow>
          ))}
        </TableBody>
        <TableFooter>
          <TableRow>
            <TablePagination
              ...
              colSpan={0}
              count={news.pagination ? news.pagination.total_count : 0}
