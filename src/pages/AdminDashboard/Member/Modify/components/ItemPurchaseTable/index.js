...
import { apiMemberItemPurchaseById } from 'services/apis/users';
...
function ItemPurchaseTable(props) {
  ...
  const getMembers = () => {
    ...
    apiMemberItemPurchaseById(props.id)
      .then(res => {
        console.log('===== res: ', res);
        if (res) {
          setMembers(res);
        ...
        <TableBody>
          {(members.pagination && (members.pagination.total_count > 0)) && members.dataset.map(member => (
            <TableRow key={member.id}>
              ...
              <TableCell>{member.item_data_lift_span + "/" + member.item_master_lift_span}</TableCell>
...
export default connect(mapStateToProps, mapDispatchToProps)(ItemPurchaseTable);
