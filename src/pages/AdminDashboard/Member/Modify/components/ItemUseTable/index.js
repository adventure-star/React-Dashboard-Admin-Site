...
import { apiMemberItemUseById } from 'services/apis/users';
          ...
          <Table className={classes.table} aria-label="Members">
            ...
            <TableBody>
              {(members.pagination && (members.pagination.total_count > 0)) && members.dataset.map(member => (
                <TableRow key={member.farm_id}>
                  <TableCell component="th" scope="row">
                    {member.farm_id}
                  </TableCell>
                  <TableCell>{member.farm_name}</TableCell>
                  <TableCell>{member.farm_data1}</TableCell>
...
export default connect(mapStateToProps, mapDispatchToProps)(ItemUseTable);
