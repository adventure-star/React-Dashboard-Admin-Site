...
import { apiEnumById } from 'services/apis/enums';
...
const headCells = [
    { id: 'enum_id', numeric: false, disablePadding: false, label: 'ID', state: false },
    { id: 'value', numeric: false, disablePadding: false, label: 'value', state: true },
    { id: 'name', numeric: false, disablePadding: false, label: '名称', state: true },
    { id: 'description', numeric: false, disablePadding: false, label: 'DESC', state: true },
    { id: 'status', numeric: false, disablePadding: false, label: 'view weight', state: true },
];
...
function DetailTable() {
    ...
    const [order_by, setOrderBy] = useState('value');
    ...
    const getEnumDetail = () => {
        ...
        apiEnumById(enumId, { per_page, page_no, order_by, order_type, ...keywords })
            .then(res => {
                console.log('===== res: ', res);
                if (res) {
                    setMembers(res);
            ...
            <Toolbar className={classes.buttonsOuterWrapper}>
                <Box className={classes.buttonsInnerWrapper}>
                    ...
                    <Tooltip arrow title="新規">
                        <Link to={{ pathname: "/AdminEnumDetailAdd", state: { id: enumId } }}>
                ...
                <TableBody>
                    {(members.pagination && (members.pagination.total_count > 0)) && members.dataset.map(member => (
                        <TableRow key={member.id}>
                            ...
                            <TableCell>
                                <Link to={{ pathname: "/AdminEnumDetailModify", state: { id: member.id } }} style={{ textDecoration: 'underline' }}>
                                    {member.value}
                                </Link>
                            </TableCell>
