...
import { apiEnums } from 'services/apis/enums';
...
const headCells = [
  { id: 'enum_id', numeric: false, disablePadding: false, label: 'ID' }
];
...
function DetailTable() {
  ...
  const [order_by, setOrderBy] = useState('enum_id');
  ...
  const getEnums = () => {
    ...
    apiEnums({ per_page, page_no, order_by, order_type, ...keywords })
      .then(res => {
        console.log('===== res: ', res);
        if (res) {
          setEnums(res);
  ...
  return (
    <TableContainer component={Paper} className={classes.tableContainer}>
      ...
      <Table stickyHeader className={classes.table} aria-label="Configs">
        ...
        />
        <TableBody>
          {(enums.pagination && (enums.pagination.total_count > 0)) && enums.dataset.map(enumitem => (
            <TableRow key={enumitem.enum_id}>
              <TableCell component="th" scope="row">
                <Link to={{ pathname: "/AdminEnumDetail", state: { id: enumitem.enum_id } }} style={{ textDecoration: 'underline' }}>
                  {enumitem.enum_id}
                </Link>
              </TableCell>
            </TableRow>
        ...
        <TableFooter>
          <TableRow>
            <TablePagination
              ...
              colSpan={0}
              count={enums.pagination ? enums.pagination.total_count : 0}
