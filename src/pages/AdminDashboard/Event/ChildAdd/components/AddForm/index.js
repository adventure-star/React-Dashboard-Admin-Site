...
import { apiCreateEvent, apiUpdateEvent } from 'services/apis/events';
...
function AddForm() {
  ...
  const onEventCreate = () => {
    ...
    apiUpdateEvent(eventId, data)
      .then(res => {
        if (res) {
          history.push('/AdminEventModify', {id: eventId})
        }
      })
  ...
  return (
    <Card className="p-4 mb-4">
      <div className="font-size-lg font-weight-bold">Add New Child Event</div>
      ...
      <div>
        ...
        <Link to={{ pathname: "/AdminEventModify", state: { id: localStorage.getItem("eventId") } }}>
