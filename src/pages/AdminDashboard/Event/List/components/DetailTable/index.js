...
import { apiEvents } from 'services/apis/events';
...
const headCells = [
  { id: 'object_id', numeric: false, disablePadding: false, label: 'イベント名' },
...
function DetailTable() {
  ...
  const getEvents = () => {
    // const {per_page, page_no, keywords} = this.state;

    apiEvents({ per_page, page_no, order_by, order_type, ...keywords })
      .then(res => {
        console.log('===== res: ', res);
        if (res) {
          setEvents(res);
  ...
  return (
    <TableContainer component={Paper} className={classes.tableContainer}>
      ...
      <Table stickyHeader className={classes.table} aria-label="Configs">
        ...
        />
        <TableBody>
          {(events.pagination && (events.pagination.total_count > 0)) && events.dataset.map(event => (
            <TableRow key={event.id}>
              <TableCell component="th" scope="row">
                <Link to={{ pathname: "/AdminEventModify", state: { id: event.id } }} style={{ textDecoration: 'underline' }}>
                  {event.object_id}:{ReactHtmlParser(event.title)}
                </Link>
              </TableCell>
              <TableCell>{event.start_date}</TableCell>
              <TableCell>{getStatus(event.delete_flag, event.start_date, event.expire_date)}</TableCell>
            </TableRow>
          ))}
        </TableBody>
        <TableFooter>
          <TableRow>
            <TablePagination
              ...
              colSpan={0}
              count={events.pagination ? events.pagination.total_count : 0}
